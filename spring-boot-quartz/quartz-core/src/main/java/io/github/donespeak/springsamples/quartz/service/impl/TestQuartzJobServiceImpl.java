...
import io.github.donespeak.springsamples.quartz.quartz.job.SplitJob;
...
import io.github.donespeak.springsamples.quartz.quartz.job.UnschedulerJob;
...
@Service
public class TestQuartzJobServiceImpl implements TestQuartzJobService {
    ...
    public void addEveryTwoSecondJob() throws SchedulerException {
        ...
        JobDetail job = JobBuilder.newJob(EveryTwoSecondJob.class)
            .withIdentity("every_two_second_" + System.currentTimeMillis(), "JOB_GROUP").requestRecovery().build();
        job.getJobDataMap().put("current", System.currentTimeMillis() + "");

        Trigger trigger = TriggerBuilder.newTrigger().withIdentity(job.getKey().getName(), "TRIGGER_GROUP")
    ...
    private void scheduleJob(String jobName, Class<? extends Job> jobClass) throws SchedulerException {
        JobDetail job = JobBuilder.newJob(jobClass).withIdentity(jobName, "JOB_GROUP").requestRecovery().build();
        job.getJobDataMap().put("current", System.currentTimeMillis() + "");
    ...
    public void addCronTriggerJob() throws SchedulerException {
        ...
        Trigger trigger = TriggerBuilder.newTrigger().withIdentity("summary_trigger", "cron").startNow()
    ...
    @Override
    public void addUnschJob() throws SchedulerException {

        JobDetail jobDetail =
            JobBuilder.newJob(UnschedulerJob.class).withIdentity("unsch_job" + System.currentTimeMillis()).build();

        Trigger trigger = TriggerBuilder.newTrigger().withIdentity(jobDetail.getKey().getName(), "TRIGGER_GROUP")
            .withSchedule(SimpleScheduleBuilder.simpleSchedule()).startNow().build();
    ...
    public void addJobThenTrigger(boolean startNow) throws SchedulerException {
        ...
        JobDetail job = JobBuilder.newJob(SplitJob.class)
            .withIdentity("spilt_job" + System.currentTimeMillis(), "JOB_GROUP")
            .requestRecovery()
            .storeDurably()
            .build();
        job.getJobDataMap().put("current", System.currentTimeMillis() + "");
        ...
        TriggerBuilder triggerBuilder = TriggerBuilder.newTrigger()
            ...
            .withSchedule(SimpleScheduleBuilder.simpleSchedule().withIntervalInMinutes(1).withRepeatCount(3))
            .forJob(job);
